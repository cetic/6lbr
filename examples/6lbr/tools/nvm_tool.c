...
create_empty_nvm(void)
{
  ...
  ipaddrconv("default", "aaaa::100", nvm_params->wsn_ip_addr);
  ipaddrconv("default", "bbbb::1", nvm_params->eth_dft_router);
...
print_nvm(void)
{
  ...
  print_bool(nvm_params->mode, CETIC_MODE_ETH_AUTOCONF);
  printf("\n");
  printf("\n");
...
main(int argc, char *argv[])
{
  ...
  if(update_nvm_file || new_nvm_file) {
    ...
    if(wsn_accept_ra) {
      mode_update(CETIC_MODE_WAIT_RA_MASK,
                  boolconv("wsn-accept-ra", wsn_accept_ra));
    }
    if(wsn_addr_autoconf) {
      mode_update(CETIC_MODE_WSN_AUTOCONF,
                  boolconv("wsn-ip-autoconf", wsn_addr_autoconf));
    }
    ...
    if(eth_ra_daemon) {
      mode_update(CETIC_MODE_ROUTER_SEND_CONFIG,
                  boolconv("eth-ra-daemon", eth_ra_daemon));
    }
    if(eth_addr_autoconf) {
      mode_update(CETIC_MODE_ETH_AUTOCONF,
                  boolconv("eth-ip-autoconf", eth_addr_autoconf));
    }
